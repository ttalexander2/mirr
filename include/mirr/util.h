...
namespace mirr::internal
{
    ...
    template<typename Type, auto Data>
    [[nodiscard]] any get_function(mirr::handle handle)
    {
        if (handle.type().id() != type_hash_v<Type>)
            return false;
    ...
    [[nodiscard]] any func_invoke([[maybe_unused]] mirr::handle handle, Func &&func,
    ...
    [[nodiscard]] any
    func_invoke([[maybe_unused]] mirr::handle handle, Func &&candidate, [[maybe_unused]] any *const args)
    {
    ...
    [[nodiscard]] any func_invoke(mirr::handle instance, any *const args)
    {
